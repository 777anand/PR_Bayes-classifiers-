...
covariance_mat=[[common_variance,0],[0,common_variance]]
...
def g(x,mean_i,covariance_mat):
	return -1*np.matmul(np.transpose(x-mean_i),np.matmul(np.linalg.inv(covariance_mat),(x-mean_i)))/2 - np.linalg.det(covariance_mat)/2
...
for i in pointsx:
	for j in pointsy:
		x=np.array([i,j])
		posterior_prob=[0,0,0]
		posterior_prob[0]=g(x,mean_c1,covariance_mat)
		posterior_prob[1]=g(x,mean_c2,covariance_mat)
		posterior_prob[2]=g(x,mean_c3,covariance_mat)
